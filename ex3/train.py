...
def init_q_table(observation_space, action_dim, discr, bool_position=None, init_q=0.):
    ...
    _shape = [ax.shape[0] for ax in axis] + [action_dim] #[16, 16, 16, 16, 2] [2]
...
def update_q_value(old_state, action, new_state, gamma, reward, done, alpha, q_axis, q_table):
    ...
    ########## Your code starts here ##########
    q_table[old_table_idx][action] = q_table[old_table_idx][action] + \
        alpha * (reward + gamma * np.max(q_table[new_table_idx]) - q_table[old_table_idx][action])
    ########### Your code ends here ##########
    return q_table
