...
class Policy(torch.nn.Module):
    def __init__(self, state_space, action_space, hidden_size=32):
        ...
        self.fc3_a = torch.nn.Linear(hidden_size, action_space)  # why not combine fc1 and fc2?
    ...
    def forward(self, x):
        # x = x.to(device)  # yibo: cuda training in consistant, 
        # no need, because in agent, self.policy  = policy.to(device)
        x_a = self.fc1_a(x)
        x_a = F.relu(x_a)
        x_a = self.fc2_a(x_a)
        x_a = F.relu(x_a)
...
class Agent(object):
    def __init__(self, policy, batch_size=64, silent=False, device = "cpu"):
    ...
    def update_policy(self):
        ...
        self.actions = torch.stack(self.actions).squeeze() # remove unnecessary singleton dimensions
    ...
    def ppo_epoch(self):
        ...
        while len(indices) >= self.batch_size:
            ...
            # Do the update
            # print(self.states.device, self.actions.device, self.rewards.device, self.next_states.device, self.dones.device, self.action_log_probs.device)
    ...
    def store_outcome(self, state, action, next_state, reward, action_log_prob, done):
        self.states.append(torch.from_numpy(state).float().to(self.train_device))
        self.actions.append(torch.Tensor([action]).to(self.train_device))
        self.action_log_probs.append(action_log_prob.detach())
        self.rewards.append(torch.Tensor([reward]).float().to(self.train_device))
        self.dones.append(torch.Tensor([done]).to(self.train_device))
        self.next_states.append(torch.from_numpy(next_state).float().to(self.train_device))
