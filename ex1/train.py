...
def main(cfg):
    ...
    device="cuda" if torch.cuda.is_available() and cfg.use_gpu == True else "cpu"
    print("device: ", device)

    # Instantiate agent and its policy
    policy = Policy(observation_space_dim, action_space_dim, device=device)
    agent = Agent(policy, cfg.batch_size, device=device)
